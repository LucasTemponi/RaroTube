...
export const SemanaSideBar: React.FC<SemanaSideBarProps> = semana => {
  ...
  return (
    <div className=' md:w-1/2 xl:w-full z-20 opacity-100 py-2 px-3 bg-gray-50 dark:bg-gray-900'>
      <ul className='  space-y-2 items-center'>
        <li className='flex items-center p-2 hover:bg-blue-100 dark:hover:bg-gray-700 rounded-lg'>
          <svg
            ...
            viewBox='0 0 256 256'
            className='dark:hover:fill-raro-rosa'
          >
            <path
              fill='#343090'
          ...
          <span
            className='cursor-pointer flex items-center p-2 w-full text-base font-bold text-raro-cobalto dark:hover:bg-gray-700 dark:text-white rounded-md '
            onClick={() => setIsOpen(!isOpen)}
          >
            {`${semana.semana.charAt(0).toUpperCase()}${semana.semana.slice(
              1
            )}`}
          </span>
        </li>
      ...
      {isOpen && (
        <ul className=' ml-4 border-l-2 space-y-2 space-x-2 border-l-raro-rosa items-center'>
          {semana.videos.map(video => {
            return (
              <Link to={`/video/${video.id}`}>
                <li className=' flex items-center p-2 pl-4 w-full text-base font-normal text-raro-cobalto dark:hover:bg-gray-700 dark:text-white rounded-lg transition duration-75 group hover:bg-blue-100'>
                  {video.nome}
                </li>
              </Link>
            );
