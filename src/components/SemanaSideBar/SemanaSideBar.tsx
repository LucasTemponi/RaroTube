...
export const SemanaSideBar: React.FC<SemanaSideBarProps> = semana => {
  ...
  return (
    <div className=' md:w-1/2 xl:w-full z-20 opacity-100 py-2 px-3 bg-gray-50 '>
      <ul className='  space-y-2 items-center'>
        <a className='flex items-center p-2 hover:bg-blue-100 rounded-lg'>
          ...
          <span
            className='cursor-pointer flex items-center p-2 w-full text-base font-bold text-raro-cobalto rounded-md '
          ...
          {semana.videos.map(video => {
            return (
              <Link to={`/video/${video.id}`}>
                <li className=' flex items-center p-2 pl-4 w-full text-base font-normal text-raro-cobalto rounded-lg transition duration-75 group hover:bg-blue-100'>
                  {video.nome}
                </li>
              </Link>
            );
